...
fn strip_ansi_codes(input: &str) -> String {
    match strip_ansi_escapes::strip(input) {
        Ok(bytes) => String::from_utf8_lossy(&bytes).to_string(),
        Err(_) => input.to_string(), // fallback to original if stripping fails
    }
}
    ...
    if is_terminal && !suppress_warnings {
        ...
        let warnings_json = serde_json::json!({
            "warnings": warnings.iter().map(|w| {
                let debug_msg = format!("{:?}", w);
                let display_msg = format!("{}", w);
                serde_json::json!({
                    "message": strip_ansi_codes(&debug_msg),
                    "display": strip_ansi_codes(&display_msg),
                    "type": "warning"
                })
            }).collect::<Vec<_>>()
        });
